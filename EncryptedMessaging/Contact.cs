...
namespace EncryptedMessaging
{
    ...
    public class Contact : INotifyPropertyChanged
    {
        ...
        /// <param name="modality">Is a server</param>
        public Contact(Context context, List<byte[]> participants, string name = null, string language = "en", RuntimePlatform os = RuntimePlatform.Undefined, string firebaseToken = null, string deviceToken = null, Modality modality = Modality.Client)
        {
        ...
        internal void Initialize(Context context = null)
        {
            ...
            if (Context.My.Modality.HasFlag(Modality.RemoveUnusedContacts) && !Modality.HasFlag(Modality.StayConnected))
                SessionTimeout = new Timer(SessionExpired, this, (uint)Context.SessionTimeout, Timeout.Infinite);
        }
        ...
        /// </summary>
        [XmlIgnore]
        public bool IsVisible => Modality.HasFlag(Modality.LoadContacts);
        ...
        internal void SetLastMessagePreview(Message lastMessage, bool ignoreLastMessageTimeCheck = false)
        {
            ...
            if (lastMessage != null && (lastMessage.ReceptionTime > LastMessageTime || ignoreLastMessageTimeCheck) && MessageFormat.MessageDescription.ContainsKey(lastMessage.Type))
            {
                ...
                {
                    var text = Encoding.Unicode.GetString(lastMessage.Data);
        ...
        public void Save(bool cloudBackup = false)
        {
            ...
            if (Context.CloudManager != null)
            {
                ...
                data = Cryptography.Encrypt(data, Context.My.Csp.ExportCspBlob(true));
                Context.CloudManager?.SaveDataOnCloud("Contact", ChatId.ToString(CultureInfo.InvariantCulture), data); // Cloud.SendCloudCommands.PostObject(Context, "Contact", ChatId.ToString(), data);
                RequestAvatarUpdate();
            }
        }
        ...
        internal void Delete() => Remove(true);

        /// <summary>
        /// Remove the contact instance
        /// </summary>
        /// <param name="deletePermanently">Delete also from storage</param>
        internal void Remove(bool deletePermanently = false)
        {
            if (SessionTimeout != null)
                SessionTimeout.Change(Timeout.Infinite, Timeout.Infinite);
            if (deletePermanently && Context.My.Modality.HasFlag(Modality.SaveContacts) && Modality.HasFlag(Modality.SaveContacts))
            {
                Context.CloudManager?.DeleteDataOnCloud("Contact", ChatId.ToString());
