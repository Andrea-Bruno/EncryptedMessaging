...
namespace EncryptedMessaging
{
    ...
    public class My
    {
        ...
        {
            {
                ...
                return new Contact(Context, participants, nameless ? null : Context.My.Name, language, Context.RuntimePlatform, FirebaseToken, DeviceToken, Modality);
            }
        }
        ...
        private CryptoServiceProvider _csp;

        /// <summary>
        /// Boolean set for the Server parameter.
        /// </summary>
        public Modality Modality => Context.Modality;
        ...
        /// <summary>
        /// Return the private key of current user in binary format (array of byte)
        /// </summary>
        /// <returns></returns>
        public byte[] GetPrivateKeyBinary() => Csp.ExportCspBlob(true);
        ...
        /// <summary>
        /// Return the private key base 64 format, stored in the device, if not present, it generates one
        /// </summary>
        /// <returns></returns>
        ...
        internal void BackupToCloud()
        {
            ...
            if (Context.Contacts.ContactsList.Count >= AntispamCloud)
            {
                if (_name != null) // Saving the name to the cloud will allow me to get it back when I recover the account with the passphrase
                    Context.CloudManager?.SaveDataOnCloud("String", "MyName", Cryptography.Encrypt(Encoding.Unicode.GetBytes(_name), Csp.ExportCspBlob(true)));  //Cloud.SendCloudCommands.PostObject(Context, "String", "MyName", SecureStorage.Cryptography.Encrypt(System.Text.Encoding.Unicode.GetBytes(_name), Csp.ExportCspBlob(true))); 
