...
namespace EncryptedMessaging
{
    ...
    public class Contacts : IDisposable
    {
        ...
        {
            Context = context;
            if (context.Modality.HasFlag(Modality.LoadContacts))
                timerRefreshLastMessageTimeDistance = new Timer(RefreshLastMessageTimeDistance, null, 60000, 60000);
        }

        private Timer timerRefreshLastMessageTimeDistance;
        ...
        internal bool RefreshSuspend
        {
            ...
            {
                if (value) _refreshSuspendCount++; else _refreshSuspendCount--; if (_refreshSuspendCount == 0)
                    SortContacts(false);
            }
        }
        ...
        public void RestoreMyContactFromCloud()
        {
            ...
            Context.CloudManager?.LoadDataFromCloud("Avatar", Context.My.Id.ToString(CultureInfo.InvariantCulture), null, true); // Cloud.SendCloudCommands.GetObject(Context, "String", "MyName");
        ...
        internal void SortContacts(bool forceUiRefresh)
        {
            ...
            {
                var sorted = SortContacts(ContactsList);
                var checker = false;
                try
                {
        ...
        private static List<Contact> SortContacts(List<Contact> contacts) => contacts.OrderByDescending(o => o.LastMessageTime).ToList();

        internal void LoadContacts(bool isRestored = false)
        {
            ...
            if (Context.My.Modality.HasFlag(Modality.LoadContacts) && Context.My.Modality.HasFlag(Modality.SaveContacts) && !isRestored)
            {
                RefreshSuspend = true;
        ...
        /// <summary>
        /// Get the total number of contacts in the address book (visible and hidden)
        /// </summary>
        public int Count => ContactsList.Count;
        ...
        {
            foreach (var contact in ContactsVisibled) //Create a new list, so that is not necessary lock when interact
        ...
        public delegate void OnContactEvent(Contact contact);

        /// <summary>
        /// Event that is triggered when new contacts are added
        /// </summary>
        public event OnContactEvent OnContactAdded;

        /// <summary>
        /// Event that is triggered when new contacts are received (For example when a contact's QR code is scanned or a client logs in to this server)
        /// </summary>
        public event OnContactEvent OnContactReceived;
        ...
        private bool AddContact(Contact contact, SendMyContact sendMyContact = SendMyContact.Send, bool sendToGroup = true, ContactMessage setSubContact = null)
        {
            ...
            lock (ContactsList)
            {
                if (ContactsList.Contains(contact))
                    return true;
                var duplicate = ContactAlreadyExists(participants, contact.Name);
                if (duplicate != null)
                {
                ...
                var backUpMyContact = Context.CloudManager != null && contact.IsVisible && ContactsList.Count == My.AntispamCloud; // When using the application is reasonable, back up my data
                if (backUpMyContact)
            ...
            else
            {
                ...
                foreach (var participant in participants)
                    ...
                    {
                        var subContact = ContactAlreadyExists(new List<byte[]> { participant }, null);
                        if (subContact == null) // exclude already exists
                        {
                            ...
                            {
                                var values = setSubContact.Participants.Find(x => x.Key.SequenceEqual(participant));
                                name = values.Name;
                                language = values.Language;
                            }
                            subContact = AddContact(new List<byte[]> { participant }, name, Modality.Client, language);
                        }
                        if (sendToGroup)
        ...
        {
            var participants = contactMessage.GetParticipantsKeys(Context);
            contactMessage.GetProperties(Context, out var language, out var os, out var firebaseToken, out var deviceToken);
            ...
            var duplicate = ContactAlreadyExists(participants, name);
            ...
            if (duplicate != null)
            {
                ...
                if (save)
                    duplicate.Save(true);
                OnContactReceived?.Invoke(duplicate);
                return duplicate;
            }
        ...
        /// <param name="sendMyContact">Option to send my contact to the contact I add</param>
        /// <returns></returns>
        public Contact AddContact(List<Contact> contacts, string name = null, Modality modality = Modality.Client, SendMyContact sendMyContact = SendMyContact.Send)
        {
            var publicKeys = "";
            foreach (var contact in contacts)
                publicKeys += contact.PublicKeys;
            return AddContact(publicKeys, name, modality, sendMyContact);
        }
        ...
        public Contact AddContact(string publicKeys, string name = null, Modality modality = Modality.Client, SendMyContact sendMyContact = SendMyContact.Send)
        {
            if (!Context.ContactConverter.PublicKeysToParticipants(publicKeys, out var participants) || !Context.ContactConverter.ValidateKeys(publicKeys))
                return null;
            var duplicate = ContactAlreadyExists(participants, name);
            if (duplicate != null)
            {
                var save = false;
                if (duplicate.Modality != modality)
                {
                    save = true;
                    duplicate.Modality = modality;
                }
        ...
        /// <param name="sendMyContact">Option to send my contact to the contact I add</param>
        /// <returns></returns>
        public Contact AddContact(List<byte[]> participants, string name = null, Modality modality = Modality.Client, string language = null, SendMyContact sendMyContact = SendMyContact.Send)
        {
            ...
            var contact = new Contact(Context, participants, name, language, modality: modality);
        ...
        public Contact GetContact(ulong chatId)
        {
            ...
            lock (ContactsList)
            {
                contact = ContactsList.ToList().Find(x => x.ChatId == chatId);
                if (contact == null)
                    contact = LoadContact(chatId);
            }
            return contact;
        }

        private Contact LoadContact(ulong chatId)
        {
            Contact contact = null;
            if (Context.Modality.HasFlag(Modality.SaveContacts))
            {
                contact = Context.SecureStorage.ObjectStorage.LoadObject(typeof(Contact), chatId.ToString(CultureInfo.InvariantCulture)) as Contact;
                if (contact != null)
                {
                    contact.Initialize(Context);
                    ContactsList.Add(contact);
        ...
        {
            var participant = GetParticipant(key);
        ...
        public Contact ContactAlreadyExists(List<byte[]> participants, string name)
        {
            ...
            var duplicate = new List<Contact>(ContactsList).Find(x =>
            {
                var p = x.Participants;
                if (p.Count != participantsClone.Count)
                    return false;
                for (var i = 0; i < p.Count; i++)
                {
                    IStructuralEquatable c = participantsClone[i];
                    if (c.Equals(p[i], StructuralComparisons.StructuralEqualityComparer) == false)
                        return false;
                }
        ...
        /// <returns>Returns the duplicate contact if it already exists, otherwise null</returns>
        public Contact ContactAlreadyExists(string publicKeys, string name) => Context.ContactConverter.PublicKeysToParticipants(publicKeys, out var participants) ? ContactAlreadyExists(participants, name) : null;
        ...
        /// <returns></returns>
        public string Pseudonym(string publicKeys) => Context.ContactConverter.PublicKeysToParticipants(publicKeys, out var participants) ? Pseudonym(participants) : null;
        ...
        /// <returns></returns>
        public string Pseudonym(ulong userId) => Pseudonym(new List<ulong>(new[] { userId }));
        ...
        public void Colors(ulong userId, out Color light, out Color dark) => Colors(new List<ulong>(new[] { userId }), out light, out dark);
        ...
        /// <param name="light"></param>
        /// <param name="dark"></param>
        public void Colors(List<byte[]> participants, out Color light, out Color dark) => Colors(ParticipantsToUserIds(participants, Context), out light, out dark);

        internal void Colors(List<ulong> userIds, out Color light, out Color dark)
        {
            var level = 0.4;
            ...
            {
                if (!darkOk)
                    dark = Color.FromArgb(rnd.Next(limit), rnd.Next(limit), rnd.Next(limit));
                if (!lightOk)
                    light = Color.FromArgb((255 - limit) + rnd.Next(limit), (255 - limit) + rnd.Next(limit), (255 - limit) + rnd.Next(limit));
