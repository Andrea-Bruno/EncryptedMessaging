...
namespace EncryptedMessaging
{
    ...
    {
        internal readonly Dictionary<DateTime, ulong> ReceptionToPostId = new Dictionary<DateTime, ulong>();
        ...
        private string ChatPath(ulong chatId) => MapPath(Path.Combine(Context.My.Id.ToString("X16", CultureInfo.InvariantCulture), Context.NetworkId.ToString("X8", CultureInfo.InvariantCulture), Context.My.Id.ToString("X16", CultureInfo.InvariantCulture), chatId.ToString("X16", CultureInfo.InvariantCulture)));
        ...
        {
            if (Context.My.Modality.HasFlag(Modality.RemoveUnusedContacts))
                return;
        ...
        private string GetFileName(DateTime dateTime, ulong chatId) => Path.Combine(ChatPath(chatId), dateTime.Ticks.ToString("X", CultureInfo.InvariantCulture) + ".post");
        ...
        public DateTime ReadPosts(ulong chatId, Action<byte[], DateTime> action = null, DateTime receptionAntecedent = default, int? take = null, List<DateTime> exclude = null)
        {
            ...
            if (Directory.Exists(path))
            {
                ...
                var skip = 0;
                if (receptionAntecedent != default)
                {
                    foreach (var file in filesList)
                    {
                        if (file.ReceptionDate < receptionAntecedent)
                            break;
                        skip++;
                    }
                }
        ...
        {
            return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), nameof(EncryptedMessaging), pathNameFile);
        }
        ...
        public static ulong PostId(byte[] dataPost)
        {
            var p1 = dataPost.Length < 8
                                ...
                                : BitConverter.ToUInt64(Bytes.Skip(dataPost, dataPost.Length - 8), 0);
            var p2 = dataPost.Length < 8
                                ? BitConverter.ToUInt64(Bytes.Skip((new byte[8]).Combine(dataPost), dataPost.Length).Reverse(), 0)
